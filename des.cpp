...
string bin_to_hex(string bin)
{
    ...
    for (size_t i = 0; i < bin.length(); i += 4)
    {
        ...
        int decimal_value = stoi(chunk, nullptr, 2);
        hex << uppercase << std::hex << decimal_value;
    }
    return hex.str();
}
...
void generate_keys(string key)
{
    ...
    cout << "Round 1 Key: " << bin_to_hex(round_keys[0]) << endl;
}
...
string DES()
{
    ...
    string xored = Xor(round_keys[0], right_expanded);
    string res = "";
    for (int i = 0; i < 8; i++)
    {
...
int main()
{
    ...
    cin >> key;
    cout << "Enter 16-character plaintext: ";
    cin >> plaintext;
    key = hexToBin(key);
    pt = hexToBin(plaintext);
    generate_keys(key);
    string ct = DES();
    cout << "Ciphertext after 1st round: " << bin_to_hex(ct) << endl;
}